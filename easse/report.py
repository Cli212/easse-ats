...
from easse.bernice import corpus_bernice
...
class ScoreBase:
    def __init__(self, sam_type='align'):
        self.sam_type=sam_type
        self.sam_score=None
        self.sam_scores = None
        self.bernice_score=None
    ...
    def get_all_scores(
        ...
        if 'sam' in metrics:
            if self.sam_score is None:
                scores['SAM'], all_scores = corpus_sam(orig_sents, sys_sents, sam=self.sam_type)
                self.sam_score = sum(all_scores) / len(all_scores)
                self.sam_scores = all_scores
            else:
                scores['SAM'] = self.sam_score
            # if len(self.sam_scores) == 0:
            #     for i, s in enumerate(scores):
            #         self.sam_scores[orig_sents[i]][sys_sents[i]] = s
            #         self.sam_score = scores['SAM']


        if 'bernice' in metrics:
            scores['BERNICE'] = corpus_bernice(orig_sents, sys_sents) if self.bernice_score is None else self.bernice_score
        if 'samsa' in metrics:
...
def get_all_scores(
    ...
    if 'sari' in metrics:
        scores['SARI'] = corpus_sari(orig_sents, sys_sents, refs_sents, tokenizer=tokenizer, lowercase=lowercase)
    if 'sam' in metrics:
        scores['SAM'] = corpus_sam(orig_sents, sys_sents, sam=sam_type)[0]
    if 'bernice' in metrics:
        scores['BERNICE'] = corpus_bernice(orig_sents, sys_sents)
    if 'samsa' in metrics:
        from easse.samsa import corpus_samsa
...
def get_qualitative_examples_html(orig_sents, sys_sents, refs_sents, score_base):
    title_key_print = [
    ...
    try:
        indexes = sorted(range(len(orig_sents)), key=lambda x: score_base.sam_scores[x], reverse=True)
        with doc.tag('div', klass='container-fluid mt-4 p-2 border'):
            collapse_id = get_random_html_id()
            with doc.tag('a', ('data-toggle', 'collapse'), ('href', f'#{collapse_id}')):
                doc.line('h3', klass='m-2', text_content="Best meaning preservation simplifications according to SAM")
            # Samples displayed by default
            with doc.tag('div', klass='collapse', id=collapse_id):
                n_samples = 50
                for i, d in enumerate(indexes):
                    if i >= n_samples:
                        break
                    doc.asis(get_one_sam_html(orig_sents[d], sys_sents[d], f'SAM={score_base.sam_scores[d]:.2f}'))

        with doc.tag('div', klass='container-fluid mt-4 p-2 border'):
            collapse_id = get_random_html_id()
            with doc.tag('a', ('data-toggle', 'collapse'), ('href', f'#{collapse_id}')):
                doc.line('h3', klass='m-2', text_content="Worst meaning preservation simplifications according to SAM")
            # Samples displayed by default
            with doc.tag('div', klass='collapse', id=collapse_id):
                n_samples = 50
...
def get_one_sam_html(orig_sent, sys_sent, print_content):
    doc = Doc()
    with doc.tag('div', klass='mb-2 p-1'):
        # Sort key
        with doc.tag('div', klass='text-muted small'):
            doc.asis(print_content)
        with doc.tag('div', klass='ml-2'):
            orig_sent_bold, sys_sent_bold = make_differing_words_bold(orig_sent, sys_sent, make_text_bold_html)
            # Source
            with doc.tag('div'):
...
def get_score_table_html_single_system(orig_sents, sys_sents, refs_sents, lowercase, tokenizer, metrics, score_base):
    return get_score_table_html_multiple_systems(
        orig_sents, [sys_sents], refs_sents, ['System output'], lowercase, tokenizer, metrics, score_base
    )


def get_score_table_html_multiple_systems(
    orig_sents, sys_sents_list, refs_sents, system_names, lowercase, tokenizer, metrics, score_base
):
    ...
    # Evaluate systems
    sys_scores_list = [
        score_base.get_all_scores(orig_sents, sys_sents, refs_sents, lowercase=lowercase, tokenizer=tokenizer, metrics=metrics)
    ...
    if len(refs_sents) > 1:
        ...
        ref_scores = score_base.get_all_scores(
            orig_sents,
            refs_sents[0],
    ...
    with doc.tag('html', lang='en'):
        ...
        with doc.tag('body', klass='container-fluid m-2 mb-5'):
            ...
            with doc.tag('div', klass='container-fluid'):
                ...
                doc.stag('hr')
                doc.asis(
                    get_score_table_html_single_system(orig_sents, sys_sents, refs_sents, lowercase, tokenizer, metrics, score_base)
                )
                doc.line('h3', 'By sentence length (characters)')
                doc.stag('hr')
                doc.asis(get_scores_by_length_html(orig_sents, sys_sents, refs_sents, metrics=metrics,sam_type=sam_type))
            doc.line('h2', 'Plots')
            doc.stag('hr')
            with doc.tag('div', klass='container-fluid'):
            ...
            doc.stag('hr')
            with doc.tag('div', klass='container-fluid'):
                doc.asis(get_qualitative_examples_html(orig_sents, sys_sents, refs_sents, score_base))
...
def get_multiple_systems_qualitative_examples_html(orig_sents, sys_sents_list, refs_sents, system_names):
    ...
    for title, sort_key, print_func in title_key_print:
        with doc.tag('div', klass='container-fluid mt-4 p-2 border'):
            ...
            with doc.tag('div', klass='collapse', id=collapse_id):
                n_samples = 50
                indexs = sorted(range(len(orig_sents)), key=lambda k:orig_sents[k], reverse=True)
                for i, d in enumerate(indexs):
                    if i >= n_samples:
                        break
                    doc.asis(get_one_sample_html(orig_sents[d], sys_sents[d], refs_sents[d], system_names, sort_key, print_func))
    return doc.getvalue()


def get_multiple_systems_html_report(
    orig_sents, sys_sents_list, refs_sents, system_names, test_set, lowercase, tokenizer, metrics, sam_type
):
    doc = Doc()
    doc.asis('<!doctype html>')
    with doc.tag('html', lang='en'):
            ...
            with doc.tag('div', klass='container-fluid'):
                ...
                doc.asis(
                    get_score_table_html_multiple_systems(
                        orig_sents, sys_sents_list, refs_sents, system_names, lowercase, tokenizer, metrics, sam_type
                    )
                )
            doc.line('h2', 'Qualitative evaluation')
            doc.stag('hr')
