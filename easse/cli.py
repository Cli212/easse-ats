...
def common_options(function):
    ...
    function = click.option(
        "--sam_type",
        "-st",
        type=click.Choice(['align', 'rc']),
        default='align',
        required=False,
        help="Which type of SAM metric, alignment or reading comprehension",
    )(function)
    return function
...
def evaluate_system_output(
    ...
    if "sam" in metrics:
        metrics_scores['sam'] = corpus_sam(orig_sents, sys_sents, sam=sam_type)[0]

    if "bernice" in metrics:
        metrics_scores['bernice'] = corpus_bernice(orig_sents, sys_sents)
