import pytest
from easse.bernice import get_bernice_score
from easse.sam import get_sam_score
from easse.utils.resources import get_orig_sents, get_refs_sents

def test_sentence_lm():
    ...
    lm_score = get_sam_score(orig_sent, sys_sent, sam='align')
    assert lm_score == pytest.approx(100.0)
    ...
    lm_score = get_sam_score(orig_sent, sys_sent, sam='align')
    assert lm_score == pytest.approx(19.0)
    ...
    lm_score = get_sam_score(orig_sent, sys_sent, sam='align')
    assert lm_score == pytest.approx(62.5)
    ...
    lm_score = get_sam_score(orig_sent, sys_sent, sam='align')
    assert lm_score == pytest.approx(14.0625)
    ...
    lm_score = get_sam_score(orig_sent, sys_sent, sam='align')
    assert lm_score == pytest.approx(50.0)


def test_corpus_lm():
    orig_sents = get_orig_sents('qats_test')
    refs_sents = get_refs_sents('qats_test')
    lm_score = get_sam_score(orig_sents, refs_sents[0], sam='rc')
    assert lm_score == pytest.approx(36.94996509406232)
