...
class BERTAligner(BERTScorer, Aligner):
    ...
    def get_sim_matrix(self, candidate, reference, sim_func):
        ...
        if sim_func == 'cosine':
            ...
            sim = sim[1:-1, 1:-1]
        elif isinstance(sim_func, dict) and "alpha" in sim_func.keys() and "beta" in sim_func.keys():
            raise NotImplemented
        elif isinstance(sim_func, dict) and "alpha" in sim_func.keys():
            softmax = torch.nn.Softmax(-1)
            alpha = sim_func.get('alpha')
            sim = (1-torch.bmm(softmax(hyp_embedding).pow(1-alpha), softmax(ref_embedding).pow(alpha).transpose(1, 2))).div_(alpha*(alpha-1))
            sim = sim.squeeze(0).cpu()
            sim = sim[1:-1, 1:-1]
        else:
    ...
    def align(self, input_text, context, sim_func):
        mat, context_tokens, input_tokens = self.get_sim_matrix(
            candidate=input_text, reference=context, sim_func=sim_func)
